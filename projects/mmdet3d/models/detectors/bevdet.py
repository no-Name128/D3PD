...
from mmdet3d.models.utils.self_print import feats_to_img, save_feature_to_img_cam
from mmdet3d.models.backbones import SwinTransformerBEVFT
...
class BEVDet(CenterPoint):
    ...
    def simple_test(self, points, img_metas, img=None, rescale=False, **kwargs):
        ...
        # import numpy as np
        # random_idx = np.random.randint(1, 100000)

        base_path = "/mnt/data/exps/D3PD/hybirdbev-review/bevdet-long-stereo"
        vis_path = f"{base_path}/vis"
        feats_to_img(
            img_feats,
            base_path=vis_path,
            suffix=f"img_feats_none_mc",
            # indice=0,
            sample_idx=img_metas[0]["sample_idx"],
        )

        # save_feature_to_img_cam(
        #     img_feats,
        #     idx=0,
        #     suffix=f"heatmap_opencv",
